...
import { Settings, Plus, Loader2, Search, X } from 'lucide-react';
    ...
    const fetchMovies = async (pageNum, isNewSearch = false) => {
        try {
            ...
            if (searchQuery) {
                ...
                url = `${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&language=en-US&with_genres=${genreId}&page=${pageNum}&sort_by=popularity.desc`;
            }
                        ...
                        <div className="flex gap-3 mt-8 overflow-x-auto no-scrollbar pb-2">
                            {GENRES.map((g) => (
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-full border text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap
                                        ${genreId == g.id || (!genreId && g.id === null) 
                                            ? 'border-[#D4AF37] text-[#D4AF37] bg-[#D4AF37]/10 shadow-[0_0_10px_rgba(212,175,55,0.2)]' 
                        ...
                        <header className="flex items-center justify-between mb-12">
                            <div className="flex items-center gap-4">
                                ...
                                <h2 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter">
                                    {searchQuery ? `FOUND IN VAULT: "${searchQuery}"` : 
                                     (GENRES.find(g => String(g.id) === String(genreId))?.name || "GENRE ARCHIVE")}
                                </h2>
                            </div>
                        </header>
